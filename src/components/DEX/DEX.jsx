...
function DEX({ chain, customTokens = {} }) {
  ...
  const { Moralis, isInitialized, chainId, logout } = useMoralis();
...
//});
  ...
  const ButtonState = useMemo(() => {
    if (chainIds?.[chainId] !== chain)
      return { isActive: false, text: `Connect Wallet` };
  ...
  useEffect(() => {
    if (currentTrade) getQuote(currentTrade).then(function(quote) {
      console.log(quote);
      setQuote(quote);
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
